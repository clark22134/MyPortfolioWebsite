...
- **Layered rate limiting** that goes well beyond a single WAF rule: in the serverless production path, AWS WAF enforces global and auth-specific thresholds at the CloudFront edge, and the Spring Boot application tracks per-IP+username attempts with a 30-minute lockout after 5 failures. (An additional Nginx protocol-level limit runs only in the local-dev Docker Compose stack — there is no Nginx in the serverless prod path.) Each layer catches what the one above it can't.
...
- An embedded **RAG chatbot** (`portfolio-chatbot-backend`) using Spring AI 1.0.8 + OpenAI that answers visitor questions with citation-backed streaming responses. The full per-query pipeline — query expansion, cosine similarity retrieval, category-aware reranking, prompt assembly, and Server-Sent Event streaming — runs inside a single Lambda function with a `SimpleVectorStore` that requires no external vector database service.
...
## 1.5 Portfolio Assistant — clarkfoster.com/chatbot

**The Problem:**
Portfolio sites communicate through static text. Recruiters and engineers reading a portfolio have questions — "Does Clark have experience with Kubernetes?", "Which project demonstrates distributed systems thinking?", "What accessibility standards does the portfolio meet?" — that a static page can't answer conversationally. Build a production-grade AI assistant that answers those questions accurately, cites its sources, and degrades gracefully when the AI provider is unavailable.

**What Makes It Technically Interesting:**
- A **full RAG pipeline** implemented with Spring AI 1.0.8: query expansion (known acronyms like ATS, RAG, IaC, CI/CD, WCAG are expanded inline to improve embedding recall), cosine similarity retrieval against a `SimpleVectorStore`, category-aware reranking that deduplicates by `(source, section)` key and applies priority weights, and prompt assembly that instructs the LLM to cite specific numbered context passages. The entire pipeline runs inside a single Lambda function — no sidecar services, no external vector DB.
- **Streaming Server-Sent Events** with a structured citation protocol. The backend emits `event: token` frames incrementally as the LLM generates the response, then a single `event: citations` frame (a JSON array of `{index, title, section, source}` records), then `event: done`. The Angular frontend consumes the SSE stream via `EventSource`, rendering tokens in real time while attaching citations as footnotes when the stream closes. This keeps p95 perceived latency low despite LLM generation time.
- **`@ConditionalOnExpression` health gating** — if `OPENAI_API_KEY` is absent or `chatbot.enabled=false`, Spring skips creating all AI beans entirely. The `GET /api/chatbot/health` endpoint returns `{ "available": false }`, and the Angular frontend hides the assistant launcher without any code change. The Lambda still starts, the main portfolio still works, and no NullPointerExceptions surface at runtime.
- **`SimpleVectorStore` over ChromaDB** — the portfolio knowledge base is bounded (hundreds of chunks, not millions). `SimpleVectorStore` runs in-process with sub-millisecond search latency, requires no additional service to provision or secure, and costs nothing beyond what Lambda's memory already provides. Swapping to `PgVectorStore`, `ChromaVectorStore`, or `PineconeVectorStore` is a single bean swap in `ChatbotConfig` because Spring AI's `VectorStore` interface hides the implementation entirely.

**Challenges Faced:**
The hardest problem was retrieval quality on a small, high-density corpus. With only a few hundred chunks, traditional top-K retrieval without reranking frequently returned duplicate information (e.g., the same project described from both the projects knowledge file and the architecture doc). The solution was category-aware reranking: after cosine similarity retrieval (TOP_K=12), chunks are deduplicated by `(source, section)` key, then ranked by category priority, and trimmed to 6 final context passages (CONTEXT_PASSAGES=6). This produces more diverse, higher-quality context windows without a cross-encoder reranker.

A secondary challenge was the `@ConditionalOnExpression` pattern with Spring AI's auto-configuration. Spring AI's starters eagerly create beans regardless of whether an API key is present, which causes startup failures when the key is missing. The fix required listing `spring.autoconfigure.exclude` for seven reactive/WebFlux auto-configurations that the Spring AI starter transitively pulls in, combined with an `ObjectProvider<RagService>` injection in the controller so graceful degradation works even if the `RagService` bean is never created.
...
**What Makes It Technically Interesting:**
- **Serverless architecture migration**: Four Spring Boot Lambda functions run in production (portfolio, dedicated portfolio-chatbot, e-commerce, ATS) behind three regional REST API Gateways, with Angular SPAs hosted on S3 + CloudFront CDN. This replaced an ECS Fargate deployment and reduced infrastructure cost by ~68% (~$200/month → ~$63/month) while improving global performance via CloudFront edge caching.
...
- **Eight Terraform modules** (~2,900 lines — acm, api-gateway, aurora, cloudfront, cloudfront-waf, lambda, networking, s3-static) provision the entire serverless stack with remote state in S3 and DynamoDB locking. (Route53 records live in the root `main.tf`, not a dedicated module.) Lambda function code is managed exclusively by GitHub Actions — Terraform manages infrastructure, not deployments. CI/CD uses **GitHub OIDC** for AWS authentication (no long-lived credentials).
...
**Challenges Faced:**
The biggest infrastructure challenge was executing the serverless migration without downtime or data loss. The migration moved from ECS Fargate (always-on containers + ephemeral sidecar databases) to Lambda + Aurora Serverless v2. The key technical hurdle was Lambda classloading: Spring Boot's default BOOT-INF JAR layout is not loadable by Lambda's URLClassLoader. The fix required switching to `maven-shade-plugin` to produce a flat uber JAR with all classes at root level. `aws-serverless-java-container-springboot3` v2.0.3 also constrains the runtime to Spring Boot 3.x, so the stack stays on the current Spring Boot 3.5.14 line (not Spring Boot 4.x). Every cost optimization has a corresponding documented tradeoff — this isn't cutting corners, it's right-sizing for actual traffic while maintaining production-grade practices.
...
**Security thinking** is systemic, not checkbox-driven: layered rate limiting (AWS WAF + Spring application in prod), HTTP-only cookies with SameSite policy, CSRF coordination across frontend and backend, file upload validation with MIME detection and UUID storage, PCI-aware card truncation, OIDC-based CI/CD with no long-lived AWS credentials.
