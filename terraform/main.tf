...
# This endpoint was adopted into Terraform state via an `import` block (PR #282) —
# an identical SES SMTP endpoint with private DNS already existed (created out-of-band
# on 2026-04-14) and AWS allows only one private-DNS endpoint per domain per VPC, so a
