...
## Languages
Java (primary), C#, Python, TypeScript, JavaScript, SQL, HTML, CSS/SCSS, Bash.

## Backend frameworks
Spring Boot 3.x, Spring Security, Spring Data JPA, Spring AI; ASP.NET Core / .NET;
FastAPI; Node.js (light use).
...
AWS: Lambda, API Gateway, CloudFront, S3, WAF, ACM, Secrets Manager,
Aurora Serverless. Terraform for all infrastructure (`terraform/` and
`terraform/modules/`). Docker and `docker-compose` for local development.
...
## Observability
Splunk, ELK stack, Spring Boot Actuator, structured JSON logging,
correlation IDs, basic distributed tracing.
...
CSRF tokens, CORS hardening, rate limiting, input validation, parameterized
queries, dependency CVE scanning.
