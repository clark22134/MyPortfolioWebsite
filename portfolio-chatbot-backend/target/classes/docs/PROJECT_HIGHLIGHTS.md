...
- An embedded **RAG chatbot** (`portfolio-chatbot-backend`) using Spring AI 1.0.5 + OpenAI that answers visitor questions with citation-backed streaming responses. The full per-query pipeline — query expansion, cosine similarity retrieval, category-aware reranking, prompt assembly, and Server-Sent Event streaming — runs inside a single Lambda function with a `SimpleVectorStore` that requires no external vector database service.
